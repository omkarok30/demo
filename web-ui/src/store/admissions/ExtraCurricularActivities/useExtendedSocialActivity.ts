...
import * as service from '@/services/admissions/ExtraCurricularActivities/extendedSocialActivity';
...
export const useExtendedSocialActivity = create(
  devtools((set: any, get: any) => ({
      ...
      try {
        ...
        _.each(res, (item) => {
          const record = convertForDisplay(
            item,
            modelExtendedSocialActivity.allColumns,
            global,
          );
          records.push(record);
        });
    ...
    getRecord: async (id = 'new') => {
      ...
      const data = convertValues(res, modelExtendedSocialActivity.allColumns);
      set({ current: data });
    },
    ...
    addRecord: async (values: any) => {
      const data = modelExtendedSocialActivity.submitJSON(values);
