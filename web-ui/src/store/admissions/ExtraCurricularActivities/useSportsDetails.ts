...
import * as service from '@/services/admissions/ExtraCurricularActivities/sportsEventDetails';
...
export const useSportsDetails = create(
  devtools((set: any, get: any) => ({
      ...
      try {
        ...
        _.each(res, (item) => {
          const record = convertForDisplay(
            item,
            modelSportsEventDetails.allColumns,
            global,
          );
          records.push(record);
        });
    ...
    getRecord: async (id = 'new') => {
      ...
      const data = convertValues(res, modelSportsEventDetails.allColumns);
      set({ current: data });
    },
    ...
    addRecord: async (values: any) => {
      const data = modelSportsEventDetails.submitJSON(values);
