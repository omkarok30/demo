...
import * as service from '@/services/admissions/contactDetails';
import * as modalContactDetails from '@/models/admissions/studentRecords/contactDetails';
import { convertForDisplay, convertValues } from '@/utils/cast';

export const useContactDetails = create(
  devtools((set: any, get: any) => ({
      ...
      try {
        ...
        _.each(res, (item) => {
          const record = convertForDisplay(
            item,
            modalContactDetails.allColumns,
            global,
          );
          records.push(record);
        });
    ...
    getRecord: async (id = 'new') => {
      ...
      const data = convertValues(res, modalContactDetails.allColumns);
      set({ current: data });
    },
    ...
    addRecord: async (values: any) => {
      const data = modalContactDetails.submitJSON(values);
