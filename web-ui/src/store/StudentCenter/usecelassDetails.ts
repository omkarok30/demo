...
import * as service from '@/services/sudentcenter/classDetails';
import * as modalCLassDetails from '@/models/studentCenter/classDetails/ClassDetails';
import { convertForDisplay, convertValues } from '@/utils/cast';

export const useClassDetails = create(
  devtools((set: any, get: any) => ({
    ...
    getRecord: async (id = 'new') => {
      ...
      const data = convertValues(res, modalCLassDetails.allColumns);
      set({ current: data });
    },
    ...
    addRecord: async (values: any) => {
      const data = modalCLassDetails.submitJSON(values);
