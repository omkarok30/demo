...
import {useprogramwise } from '@/store/fyacademics/dashboad/useprogramwise';
...
const EmployeeDashboardList = () => {
  ...
  const storeprogramwise = useprogramwise((state: any) => ({
  ...
  const handleActionChange = (event) => {
    setYear(event);
    storeprogramwise.getRecords(event);
  };
  ...
  React.useEffect(() => {
    fetchSettings();
    storeprogramwise.getRecords();
  }, []);

  const columns: ColumnsType<any> = React.useMemo(() => {
    const cols = modelFyacademicdashboard.programwise(settings);
    ...
    <div className="layout-main-content">
      <Card bordered={false} title="">
        <Form.Item>
          ...
          <Space>
            Program
            <Select style={{ width: 200 }} options={optionaprogram} />
          </Space>
        ...
        <Table
          bordered
          columns={columns}
          dataSource={storeprogramwise.allRecords}
        />
      </Card>
    </div>
  );
