...
import { usedivisionwise } from '@/store/fyacademics/dashboad/usedivisionwise';
...
const EmployeeDashboardList = () => {
  ...
  const optiondivision = todoLookUps.getState().division;
  
  const [selectYear, setYear] = useState();
  const storedivisionwise = usedivisionwise((state: any) => ({
  ...
  const handleActionChange = (event) => {
    setYear(event);
    storedivisionwise.getRecords(event);
  };
  ...
  React.useEffect(() => {
    fetchSettings();
    storedivisionwise.getRecords();
  }, []);

  const columns: ColumnsType<any> = React.useMemo(() => {
    const cols = modelFyacademicdashboard.divisionwise(settings);
    ...
    <div className="layout-main-content">
      <Card bordered={false} title="">
        <Form.Item>
          ...
          <Space>
        Division
            <Select style={{ width: 200 }} options={optiondivision} />
          </Space>
        ...
        <Table
          bordered
          columns={columns}
          dataSource={storedivisionwise.allRecords}
        />
      </Card>
    </div>
  );
