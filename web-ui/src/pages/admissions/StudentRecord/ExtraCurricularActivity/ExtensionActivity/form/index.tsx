...
const ExtensionActivityForm = ({
  ...
  const schemaRules = React.useMemo(
    () =>
      schemaValidator(modelExtensionActivity.schemaRules, {
  ...
  const optionsAcheivement = todoLookUps.getState().achievementPrice;
  const optionsYesOrNo = todoLookUps.getState().yesNo;
  const optionCountry = todoLookUps.getState().country;
  const typeOfExtensionActivity = todoLookUps.getState().typeOfExtensionActivity;
  const dummyMultiSelectOptions = todoLookUps.getState().dummyMultiSelectOptions;
  ...
  const [displayOtherActivity, setDisplayOtherActivity] = useState(false);
  const [displayOtherAgency, setDisplayOtherAgency] = useState(false);
  ...
  const onChangeActivity = (event: any) => {
    if (event === 'other') {
      setDisplayOtherActivity(true);
    }
    else {
      setDisplayOtherActivity(false);
    }
  };

  const handleAddionalAgency = (event: any) => {
    if (event === 'true') {
      setDisplayOtherAgency(true);
    }
    else {
      setDisplayOtherAgency(false);
    }
  };

  const headerLabel = isNew
    ? 'Add Extension Activity Details'
    : 'Edit Extension Activity Details';
      ...
      <Form form={form} layout="vertical" disabled={saveProgress.disableForm}>
          ...
          <Row className="justify-center">
            <Col className="w-md">
              ...
              >
                <Select
                  options={typeOfExtensionActivity}
                  onChange={onChangeActivity}
                />
              </Form.Item>
              <When condition={displayOtherActivity}>
                <Form.Item
              ...
              >
                <Select
                  ...
                  onChange={handleAddionalAgency}
                />
              </Form.Item>
              <When condition={displayOtherAgency}>
                <Form.Item
              ...
              <Form.Item
                ...
                label="Faculty Coordinator/ Participation"
                rules={schemaRules}
                required
              >
                <Select
              ...
              <Form.Item
                name="achievementAwardDetails"
                label="Achievement/Award Details"
                rules={schemaRules}
                required
              >
                <Select options={optionsAcheivement} />
...
export default ExtensionActivityForm;
