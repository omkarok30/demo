    ...
    return (
        <div>
            <Form form={form} layout="vertical" >
                <Form.Item name="Resignation Date" label="Resignation Date" required style={{ flex: 1, marginRight: 10, fontWeight: 'bold', marginTop: 20 }}>
                    <DatePicker className="w-100%" format={global.displayDateFormat} placeholder={'PLEASE SELECT FROM CALENDAR'} />
                </Form.Item>
                <Form.Item name="reasonForDeactivation" label="Reason for Resignation" required style={{ flex: 1, marginRight: 10, fontWeight: 'bold', marginTop: 20 }}>
                    <Input.TextArea style={{ textTransform: 'uppercase' }} rows={3} />
                </Form.Item>

                <Form.Item name="Relieving Date" label="Relieving Date" required style={{ flex: 1, marginRight: 10, fontWeight: 'bold', marginTop: 20 }}>
                    <DatePicker className="w-100%" format={global.displayDateFormat} placeholder={'PLEASE SELECT FROM CALENDAR'} />
                </Form.Item>
                <Form.Item name="Upload Resignation Letter/Copy" label="Upload Resignation Letter/Copy" getValueFromEvent={getFile} style={{ flex: 1, marginRight: 10, fontWeight: 'bold' }}>
                    <Upload>
                        <Button icon={<UploadOutlined />}>Choose File</Button>
                    </Upload>
                </Form.Item>
                <Form.Item name="Relieving Letter" label="Relieving Letter" getValueFromEvent={getFile} style={{ flex: 1, marginRight: 10, fontWeight: 'bold' }}>
                    <Upload>
                        <Button icon={<UploadOutlined />}>Choose File</Button>
                    </Upload>
                </Form.Item>
