...
const NaacStudent21 = () => {
  ...
  const columns: ColumnsType<any> = useMemo(() => {
    let cols = modelNaacStudent21.columns(settings);
    cols[1] = {
      dataIndex: 'studentPromotionMap$count',
      title: 'Number of students enrolled',
      width: 300,
      render: (_, record) => record.studentPromotionMap$count ? [<Link to={`../edit/?year=${record.academicYear}`}>{record.studentPromotionMap$count}</Link>, <Button type='link' icon={<DownloadOutlined title='Download' />} />] : '-',
    };
