...
const ManageCourseEdit = () => {
  ...
  const defaultProgramValue = storeProgramDetails.allRecords?.filter(
    (record: any) => record.id === storeManageCourses.current.programId
  );
  const optionsDefaultProgramValue = _.map(defaultProgramValue, (record) => ({
    value: record.id,
    label: `${record.programmeName} (${record.programCode})`
  }));
  // storeManageCourses.current.programId = optionsDefaultProgramValue[0]?.label;
  const optionsCurriculumComponentValue = optionsCurriculumComponent?.filter(
    (record: any) => record.value === storeManageCourses.current.curriculumComp
  );
  React.useEffect(() => {
    ...
    if (storeManageCourses.current.courseMethdCredits) {
      ...
      CourseMethodValue.forEach((element, index) => {
        let obj = courseMethdCredits.find(
    ...
    if (storeProgramDetails.allRecords) {
      ...
      if (storeProgramDetails.allRecords) {
        if (storeManageCourses.current.programId) {
          let value = storeProgramDetails.allRecords.find(
  ...
  const selectDegreeLevel = (value: any) => {
    const programDetailss = storeProgramDetails.allRecords?.filter(
      (record: any) => record.levelOfEducation === value
    );
    const optionsProgramDetailss = _.map(programDetailss, (record) => ({
      value: record.id,
      label: `${record.programmeName} (${record.programCode})`
    }));
    setProgramDetails(optionsProgramDetailss);
  ...
  const onCreaditsSubmit = () => {
    let courseMethod: any = form.getFieldValue("courseMethod");
    let creadits: any = 0;
  ...
  const headerLabel = isNew ? "Add Course" : "Edit Course";
      ...
      <Form form={form} layout="vertical" disabled={saveProgress.disableForm}>
        ...
        >
          <Row className="justify-center">
            <Col className="w-md">
              <When condition={!isNew}>
                <Form.Item name="programId" label="Program" required>
                  <Input disabled />
                </Form.Item>
              </When>
              <When condition={isNew}>
              ...
              <Form.List
                name="courseMethod"
                initialValue={CourseMethodValue}
              >
                {(fields) => (
                  <div>
