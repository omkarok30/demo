...
const BankDetailsFormEdit = () => {
  const { id } = useParams();
  const isNew = id === 'new';

  const global = useGlobalState((state: any) => state.default);
  const settings = useSettings((state: any) => ({ byKeys: state.byKeys, asSelect: state.asSelect }));
  const schemaRules = React.useMemo(() => schemaValidator(modelBankDetails.schemaRules, { settings: settings.byKeys }), [settings.byKeys]);

  const storeBankDetails = useBankDetails(
    (state: any) => ({
  ...
  React.useEffect(() => {
    if (storeBankDetails.current.id !== id) {
      return;
    }
    form.setFieldsValue(storeBankDetails.current);
  }, [storeBankDetails.current]);
  ...
  const onFormSubmit = () => {
    form.validateFields()
      .then(async (values) => {
        ...
        if (isNew) {
          const record = await storeBankDetails.addRecord(values);
          if (!isEmptyValue(record)) {
            notification.success({ message: 'Saved Successfully!', description: `Created record for ${record.bankName}` });
          }
        }
        else {
          const record = await storeBankDetails.updateRecord(id, values);
          if (!isEmptyValue(record)) {
        ...
        <Row className="justify-center">
          <Col className='w-md'>
            <When condition={!isNew}>
              {() => (<>
                <Form.Item name="bankName" label="Bank Name" rules={schemaRules} required>
                  <Input />
                </Form.Item>
                <Form.Item name="branchName" label="Branch" rules={schemaRules} required>
                  <Input />
                </Form.Item>
                <Form.Item name="ifscCode" label="IFSC Code" rules={schemaRules} required>
                  <Input />
                </Form.Item>
                <Form.Item name="accountNumber" label="Account Number" rules={schemaRules} required>
                  <Input />
                </Form.Item>
                <Form.Item name="branchAddress" label="Branch Address" rules={schemaRules}>
                  <Input />
                </Form.Item>
                <Form.Item name="linkToSalary" label="Link to Salary" rules={schemaRules} >
...
export default BankDetailsFormEdit;
